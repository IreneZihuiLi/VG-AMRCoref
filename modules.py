...
class TokenEncoder(nn.Module):
    def __init__(self, token_vocab, char_vocab, char_dim, token_dim, embed_dim, filters, char2token_dim, dropout,bert_dim=None):
        ...
        'if using bert (model1)'
        # self.bert_mapping = nn.Linear(768,token_dim)

    def reset_parameters(self):
        nn.init.normal_(self.out_proj.weight, std=0.02)
        nn.init.constant_(self.out_proj.bias, 0.)

    def forward(self, token_input, char_input,bert_input=None):
        ...
        #     bert_token = self.bert_mapping(bert_input).unsqueeze(1)  # [con_len,256]
        #     token_repr += bert_token
        'bert model2'
        if bert_input is not None:
            bert_token = bert_input.unsqueeze(1)
            token_repr = torch.cat([token_repr,bert_token],-1)
