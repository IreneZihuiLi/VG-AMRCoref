...
def evaluate(model, dataset, log_file, args, finished_epochs, output_path=None, reference_path=None):
    model.eval()
    ...
    gold_labels_for_analysis = []
    pred_labels_for_analysis = []
    mention_ids_for_analysis = []
    ...
    for step, ori_batch in enumerate(dataset):
        ...
        pred_labels_for_analysis.append(outputs['overall_argmax'][0].detach().cpu().numpy())
        gold_labels_for_analysis.append(outputs['mention_cluster_ids'][0].detach().cpu().numpy())
        mention_ids_for_analysis.append(outputs['mention_ids'][0].detach().cpu().numpy())
        # import pdb;pdb.set_trace()

        'analysis ends'
        # print ('epoch {}, now eval {}, len {}'.format(finished_epochs,step,len(pred_labels_for_analysis)))
    ...
    print("----Average F1 (py): {:.3f}% precision (py): {:.3f}% recall (py): {:.3f}% on {} docs"
          .format(f[0] * 100, p * 100, r * 100, len(dataset)))

    log_file.write("Average F1 (py): {:.2f}% on {} docs\n".format(f[0] * 100, len(dataset)))
    log_file.write("All F1: MUC,B3,CLEFA, AVG:\n")
    log_file.write("All F1s:  {:.2f}%, {:.2f}%, {:.2f}%, {:.2f}%, \n".format(f[1] * 100, f[2] * 100,f[3] * 100, f[0] * 100))
    ...
    # import pdb;
    # pdb.set_trace()
    with open(args.analysis_path+'/truth.'+str(finished_epochs),'w') as file:
        for line,x in enumerate(gold_labels_for_analysis):
            file.write('\t'.join([str(int(y)) for y in x]) + '\n')


    with open(args.analysis_path+'/pred.'+str(finished_epochs),'w') as file:
        for x in pred_labels_for_analysis:
            file.write('\t'.join([str(int(y)) for y in x]) + '\n')

    with open(args.analysis_path + '/metion_ids.' + str(finished_epochs), 'w') as file:
        for x in mention_ids_for_analysis:
            file.write('\t'.join([str(int(y)) for y in x]) + '\n')
    'analysis ends'
